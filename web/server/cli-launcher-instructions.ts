...
- **Use quest threads for quest-scoped context.** Main is the staging area for unthreaded/global work. Quest-backed threads carry quest-specific activity, and All Threads/global inspection preserves the append-only audit stream. Use \`takode thread attach\` when needed to organize clearly quest-specific prior discussion; automatic answer routing never requires a separate answer attachment or history-index lookup.
