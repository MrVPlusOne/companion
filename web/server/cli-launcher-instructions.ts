...
- Base branch / port target: \`${syncBaseBranch}\`${portTargetWorktree}${portTargetSource}`);
  ...
  parts.push(
    "## Session Timers\n\n" +
      ...
      "Leader timers save a destination with `--thread main` or `--thread q-N`. Without `--thread`, creation inherits a valid active-turn route only while the leader is generating; otherwise specify the destination explicitly. Later thread changes do not reroute the timer. Existing unassociated timers retain their behavior. Each actual firing supplies a distinct `id:fN` optional answer reference; recurring timer `tN` is not an answer reference. Use `:C` for progress or bookkeeping and `:A:fN` for a substantive scheduled report, preserving unrelated pending user requests.\n\n" +
