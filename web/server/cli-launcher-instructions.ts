...
- Base branch / port target: \`${syncBaseBranch}\`${portTargetWorktree}${portTargetSource}`);
  ...
  parts.push(
    "## Session Timers\n\n" +
      ...
      "Leader timers save a destination with `--thread main` or `--thread q-N`. Without `--thread`, creation inherits a valid active-turn route only while the leader is generating; otherwise specify the destination explicitly. Later thread changes do not reroute the timer. Existing unassociated timers retain their behavior. Each actual firing supplies a distinct `id:timer-mN` optional answer reference; recurring timer `tN` is not an answer reference. Use `:C` for progress or bookkeeping and `:A:timer-mN` for a substantive scheduled report, preserving unrelated pending user requests. Older supplied `fN` references retain their exact spelling; never substitute `timer-mN` for a historical reference.\n\n" +
...
A substantive scheduled report may answer a supplied timer-firing ID with \`[thread:main:A:timer-m1]\` or \`[thread:q-N:A:timer-m2]\`; mixed references such as \`[thread:q-N:A:u1,timer-m2]\` are valid. Each firing's \`id:timer-mN\` is distinct from recurring timer \`tN\`. Older supplied \`fN\` references retain their exact spelling; never substitute \`timer-mN\` for a historical reference. Never invent a human ID or use \`tN\` as an answer target. Progress and bookkeeping remain commentary. Timer firings are optional answer targets and do not create pending human requests; answering them leaves unrelated requests pending. Useful complementary reports follow the same answer-quality rule.
