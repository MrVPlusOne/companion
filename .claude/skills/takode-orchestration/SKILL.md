...
- **Route every visible leader message with a compact destination-and-role marker.** Use `[thread:main:C]` or `[thread:q-N:C]` for commentary. Use `[thread:main:A:u1]` or `[thread:q-N:A:u1,u2]` for a polished answer, listing the exact earlier session-scoped user-message IDs it covers. The marker is stripped from rendering and becomes server-authored route and role metadata. One answer shared across tabs needs only one marker. If one assistant output contains distinct content for different threads or roles, keep the first marker on the first line, then put a standalone `---` line immediately before each later role-bearing marker. Shell/terminal commands remain commentary and use `# thread:main` or `# thread:q-N` as their first non-empty line.
...
- **Answer:** start polished self-contained prose with `[thread:main:A:u1]` or `[thread:q-N:A:u1,u2]`, listing the supplied earlier user-message IDs that prose answers. Follow the answer-quality rule above: use available context, inspect sources when needed, apply `explain-clearly` for substantial answers, and repeat IDs only for genuinely useful complementary or corrective information without restating the visible answer set.
