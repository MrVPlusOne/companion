...
- **Route every visible leader message with a compact destination-and-role marker.** Use `[thread:main:C]` or `[thread:q-N:C]` for commentary. Use `[thread:main:A:u1]` or `[thread:q-N:A:u1,u2]` for a polished answer, listing the exact supplied earlier session-scoped IDs it covers (`uN` for human messages, `fN` for individual timer firings). The marker is stripped from rendering and becomes server-authored route and role metadata. One answer shared across tabs needs only one marker. If one assistant output contains distinct content for different threads or roles, keep the first marker on the first line, then put a standalone `---` line immediately before each later role-bearing marker. Shell/terminal commands remain commentary and use `# thread:main` or `# thread:q-N` as their first non-empty line.
...
- **`[Timer reminder HH:MM id:fN]`** -- one actual timer firing; `fN` is its optional answer target, distinct from recurring timer `tN`; inspect it when needed with `takode read <leader-session> fN`
...
- **Answer:** start polished self-contained prose with `[thread:main:A:u1]` or `[thread:q-N:A:u1,u2]`, listing the supplied earlier human `uN` or timer-firing `fN` IDs that prose answers. Follow the answer-quality rule above: use available context, inspect sources when needed, apply `explain-clearly` for substantial answers, and repeat IDs only for genuinely useful complementary or corrective information without restating the visible answer set.
- **Timer reports:** a substantive scheduled report may use `[thread:main:A:f1]`, `[thread:q-N:A:f2]`, or a mixed set such as `[thread:q-N:A:u1,f2]`. Use only supplied firing IDs; never invent a human ID or use recurring `tN` as an answer target. Progress, checks without a substantive result, and bookkeeping remain `:C`. A firing is optional to answer and creates no pending human request; answering it leaves unrelated user requests pending. Complementary answers follow the same rule as human answers.
...
Each actual delivered firing supplies an `id:fN` source reference. Recurring firings share timer `tN` but receive distinct firing references. Do not access another app or start external work merely because a timer fired; existing task authority and permissions still apply.
