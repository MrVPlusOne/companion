...
- If the task, scope, or claimed behavior is ambiguous enough that you cannot review confidently, ask the leader in plain text first, then call `takode notify needs-input` with a short summary and suggested replies under the `takode-orchestration` notification rule, and stop instead of guessing.
